...
const deleteSelectChirp = `-- name: DeleteSelectChirp :exec
DELETE FROM chirps WHERE id = $1
`

func (q *Queries) DeleteSelectChirp(ctx context.Context, id uuid.UUID) error {
	_, err := q.db.ExecContext(ctx, deleteSelectChirp, id)
	return err
}
...
const retrieveSelectChirp = `-- name: RetrieveSelectChirp :one
SELECT id, created_at, updated_at, body, user_id FROM chirps WHERE id = $1
`

func (q *Queries) RetrieveSelectChirp(ctx context.Context, id uuid.UUID) (Chirp, error) {
	row := q.db.QueryRowContext(ctx, retrieveSelectChirp, id)
	var i Chirp
	err := row.Scan(
		&i.ID,
		&i.CreatedAt,
